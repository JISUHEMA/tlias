...
public class UserController {
	...
	public String register(User user,String code,HttpSession session) throws UnsupportedEncodingException {
		...
		log.info("ユーザー名:{},名前:{},パスワード:{},性別:{}:{}",user.getUsername(),user.getRealname(),user.getPassword(),user.isGender(),code);
		
		try {
			String sessionCode = session.getAttribute("code").toString();
			
			if(!sessionCode.equalsIgnoreCase(code)) throw new RuntimeException("検証コードが間違っています");
			
			userService.register(user);
		} catch (Exception e) {
			
			e.printStackTrace();
			return "redirect:/registry?msg="+URLEncoder.encode(e.getMessage(),"UTF-8");
		}
		return "redirect:/login";
	}
